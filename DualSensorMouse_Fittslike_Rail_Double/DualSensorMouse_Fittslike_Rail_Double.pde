...
void draw() {
  ...
  textAlign(CENTER, CENTER);
  text(cnt + "/" + cycle*2, width/2, height/2);
...
void OnClick() {
  ...
  if (cnt > 1) {
    ...
    if (senPos < 1.0 && senPos >= 0) {
      String Log = current_exp.toString().replace("_", ",") + "," + (cnt-1) + "," + cycleRepeat + "," + String.format("%.2f", senPos) + "," + ("T");
      Pos_Logger.println(Log);
      Pos_Logger.flush();
    } else {
      String Log = current_exp.toString().replace("_", ",") + "," + (cnt-1) + "," + cycleRepeat + "," + String.format("%.2f", senPos) + "," + ("F");
      Pos_Logger.println(Log);
      Pos_Logger.flush();
    }
  }
